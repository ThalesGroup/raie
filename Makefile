...
lint: clean ## check style, typing and formatting
	@echo "> Lint Sources"
	@echo ">> Static Linting"
...
inits: # Create init files with mkinit
	@echo ">> Creating __init__ files"
	@poetry run mkinit raie --recursive --write --black
...
test: clean
	@echo "> Pass Tests"
	@pytest . --junitxml=build/reports/tests.xml --cov=src/raie --cov-report=term-missing --cov-report=xml:build/reports/coverage.xml --cov-report=html:build/reports/cov --cov-branch -rsx
	@xdoctest raie
